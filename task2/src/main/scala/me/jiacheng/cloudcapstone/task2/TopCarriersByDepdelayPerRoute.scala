...
object TopCarriersByDepdelayPerRoute {
  def mapLine2Fields(line: (String, String)) : ((String, String, String), Int) = {
    ...
    ((record.get(Setup.COL_ORIGIN), record.get(Setup.COL_DEST), record.get(Setup.COL_CARRIER)), delay)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val airportCarrier2delay = KafkaUtils.createStream(ssc, zkQuorum, group, topicMap)
      .map(mapLine2Fields)
      .reduceByKey(_ + _)
      .map{ case ((src, dst, carrier), delay) => ((src, dst), (carrier, delay)) }
      .groupByKey()
      .map{ case ((src, dst), carriervals) =>
        val topCarriers = carriervals.toList.sortBy(_._2).take(10)
        (src, dst, topCarriers.map(_._1), topCarriers.map(_._2))
      }
