...
public class TopCarriersByOntimeDeparturePerAirport extends Configured implements Tool {
    public static class MapCompositeCount extends Mapper<LongWritable, Text, TextArrayWritable, IntWritable> {
        private static final int[] columns = {Setup.COL_ORIGIN, Setup.COL_CARRIER, Setup.COL_DEPDELAYM};
        public void map(LongWritable key, Text val, Context context)
            ...
            for (CSVRecord record: parser) {
                ...
                String carrier  = record.get(columns[1]);
                String delay    = record.get(columns[2]);

                if (orig.isEmpty() || carrier.isEmpty() || delay.isEmpty()) continue;

                context.write(
                        new TextArrayWritable(new String[] {orig, carrier}),
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new TopCarriersByOntimeDeparturePerAirport(), args);
        System.exit(res);
    }
    ...
    public final int run(String[] args) throws Exception {
        ...
        job = new Job(conf, "airport-carrier departure ontime performance");
        job.setJarByClass(TopCarriersByOntimeDeparturePerAirport.class);
        ...
        job = new Job(conf, "top airport-carrier departure ontime performance");
        job.setJarByClass(TopCarriersByOntimeDeparturePerAirport.class);
