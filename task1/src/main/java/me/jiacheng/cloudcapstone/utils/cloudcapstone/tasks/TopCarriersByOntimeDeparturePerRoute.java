...
public class TopCarriersByOntimeDeparturePerRoute extends Configured implements Tool {
    public static class MapCompositeCount extends Mapper<LongWritable, Text, TextArrayWritable, IntWritable> {
        private static final int[] columns = {Setup.COL_ORIGIN, Setup.COL_DEST, Setup.COL_CARRIER, Setup.COL_DEPDELAYM};
        public void map(LongWritable key, Text val, Context context)
            ...
            for (CSVRecord record: parser) {
                ...
                String carrier  = record.get(columns[2]);
                String delay    = record.get(columns[3]);

                if (orig.isEmpty() || dest.isEmpty() || carrier.isEmpty() || delay.isEmpty()) continue;

                context.write(
                        new TextArrayWritable(new String[] {orig, dest, carrier}),
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new TopCarriersByOntimeDeparturePerRoute(), args);
        System.exit(res);
    }
    ...
    public final int run(String[] args) throws Exception {
        ...
        job = new Job(conf, "route-carrier departure ontime performance");
        job.setJarByClass(TopCarriersByOntimeDeparturePerRoute.class);
        ...
        // for route: orig-dest, keylength = 2
        conf.setInt("keyLength", 2);
        job = new Job(conf, "top route-carrier departure ontime performance");
        job.setJarByClass(TopCarriersByOntimeDeparturePerRoute.class);
